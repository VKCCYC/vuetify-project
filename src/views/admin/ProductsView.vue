  ...
  VRow
    ...
    VCol(cols="12")
      VDataTableServer(
        ...
        template(#top)
          VTextField(
          label="搜尋" append-icon="mdi-magnify"
          v-model="tableSearch"
        ...
        template(#[`item.edit`]="{ item }")
          VBtn(icon="mdi-pencil" variant="text" color="pink" @click="openDialog(item)")
      //- persistent 讓他固定
VDialog(v-model="dialog" persistent width="600px")
...
const submit = handleSubmit(async (values) => {
  // 表示沒有圖片 fileRecords.value.length ===0
